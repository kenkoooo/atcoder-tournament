...
const fetchTournamentSeason12 = async (): Promise<TournamentResponse> => {
  const usersResponse = await fetch("https://abc-api.kenkoooo.com/api/users");
  const usersText = await usersResponse.text();

  const previousBracketsResponse = await fetch("./bracket-11.json");
...
export const useTournament = (seasonId: string) => {
  const fetcher = (url: string) => {
    if (seasonId === "12") {
      return fetchTournamentSeason12();
    } else {
      return fetch(url)
